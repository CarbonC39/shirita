...
async fn test_state() -> (AppState, std::path::PathBuf) {
    ...
    let state = AppState { storage, config, provider, token_counter, model: "m".into(), generations: Arc::new(shirita_web::Generations::new()), http_client: shirita_web::new_http_client(), authorization: Arc::new(shirita_core::mcp::authorization::AuthorizationBroker::new()) };
    (state, assets)
}
