import type { MessageSchema } from './en'

const zhHant: MessageSchema = {
  common: {
    approve: '批准',
    deny: '拒絕',
    save: '儲存',
    cancel: '取消',
    delete: '刪除',
    duplicate: '複製',
    rename: '重新命名',
    add: '新增',
    close: '關閉',
    import: '匯入',
    export: '匯出',
    loading: '載入中…',
    saved: '已儲存',
    saving: '儲存中…',
    done: '完成',
    tokensEstimate: '~{tokens} tokens',
    avatarPickHint: '從圖庫選擇或上傳新圖片。',
    avatarCropHint: '將頭像裁剪為正方形。',
    imageDeleteConfirm: '從圖庫刪除此圖片？',
    imageNone: '無圖片',
    imageName: '圖片名稱',
    imageDelete: '從圖庫刪除',
    uploading: '上傳中…',
    imageUpload: '上傳圖片',
    fullscreenEditor: '全螢幕編輯器',
    startTyping: '開始輸入…',
  },
  shell: {
    chats: '對話',
    new: '新增',
    book: '設定集',
    settings: '設定',
  },
  home: {
    empty: '尚無對話。點 + 新增一個開始。',
    importTitle: '匯入對話',
    newChatAria: '新增對話',
    done: '完成',
    reorderDelete: '重新排序與刪除',
    deleteConfirm: '刪除此對話及其所有訊息？',
  },
  newChat: {
    title: '新增對話',
    namePlaceholder: '名稱（選填）',
    template: '範本',
    templatePlaceholder: '選擇範本…',
    newTemplate: '新增範本（前往書）',
    mountPacks: '掛載包',
    mountPlaceholder: '新增包…',
    newPack: '新增包（前往書）',
    avatar: '頭像（選填）',
    create: '建立對話',
    creating: '建立中…',
    removePack: '移除',
    reorderPack: '拖曳以排序',
    leaveConfirm: '放棄未儲存的內容？本次新增對話的編輯將遺失。',
  },
  prompt: {
    deleteContainerConfirm: '刪除此容器及其中的 {count} 個項目？',
    deleteNodeConfirm: '刪除提示節點"{name}"？',
    addNode: '新增節點',
    omniPlaceholder: '輸入以新增提示詞或容器…',
    omniNewPrompt: '新增提示詞「{name}」',
    pickerSearch: '搜尋…',
    pickerSearchTyped: '搜尋 {type}…',
    pickerNewTyped: '新增 {type}…',
    pickerNewDefault: '新增定義…',
    moreCount: '還有 {count} 個',
    noMatching: '沒有相符的定義',
    otherType: '其他類型',
    chatHistory: '聊天記錄',
    contentMount: '掛載的包',
    selectAll: '全部',
    selectOne: '單選',
    selectModeHint: '全部：渲染所有啟用項；單選：只渲染第一項。',
    folderFallback: '(資料夾)',
    missing: '(遺失)',
    addToContainer: '新增至容器',
    trigger: '觸發',
    triggerConstant: '常駐',
    triggerKeyword: '關鍵詞',
    triggerRandom: '隨機',
    addKeyword: '新增關鍵詞…',
    probability: '機率 %',
  },
  chat: {
    requiresApproval: '需要你的批准',
    back: '返回',
    title: '對話',
    assistant: '助手',
    you: '你',
    regenerate: '重新生成',
    fork: '分支',
    copy: '複製',
    copied: '已複製',
    copyFailed: '複製失敗——瀏覽器可能封鎖了此操作。',
    edit: '編輯',
    hide: '隱藏',
    unhide: '取消隱藏',
    previousVariation: '上一個變體',
    nextVariation: '下一個變體',
    delete: '刪除',
    deleteConfirm: '刪除這則訊息及其所有回覆？此操作無法復原。',
    empty: '尚無訊息。在下方輸入開始對話。',
    noMessages: '尚無訊息',
    dragReorder: '拖曳以重新排序',
    options: '選項',
    thinking: '思考過程',
    forkNotice: '已分叉對話 — 這是一個獨立副本。',
    retry: '重試',
    dismiss: '關閉',
    details: '詳情',
  },
  composer: {
    attach: '附件',
    send: '傳送',
    removeAttachment: '移除附件',
    placeholder: '輸入訊息…',
    stop: '停止生成',
    uploadFailed: '圖片上傳失敗——請重試。',
  },
  book: {
    localHeading: '本對話',
    revertToGlobal: '還原為全域',
    customizeLocally: '本機自訂',
    showGlobalLibrary: '顯示全域庫',
    hideGlobalLibrary: '隱藏全域庫',
    variablesThisChat: '變數（本對話）',
    newTemplate: '+ 新增範本',
    importConflictFound: '{skipped} 個項目因名稱衝突已跳過，是否改為覆寫？',
    conflictOverwrite: '覆寫',
    conflictDuplicate: '複本',
    importSummary: '已匯入：新增 {created} 個，跳過 {skipped} 個，覆寫 {overwritten} 個。',
    importFailed: '匯入失敗——請檢查檔案後重試。',
    importPanelDetected: '偵測到狀態列，已產生原生面板預覽。',
    templateNamePlaceholder: '範本名稱',
    deleteTemplateConfirm: '刪除此範本？',
    deleteTemplateOrphans: '此範本是唯一引用這 {count} 個定義的地方。是否一併刪除？',
    templateHeading: '範本',
    defaultTemplate: '預設範本',
    packHeading: '包',
    createPack: '新增包',
    packNamePlaceholder: '包名稱',
    deletePackConfirm: '刪除這個包？',
    deleteDefConfirm: '刪除定義"{name}"？',
    nav: {
      back: '返回',
      backToTemplate: '返回範本',
      backToPack: '返回設定包',
    },
  },
  pack: {
    identity: '身分',
    displayName: '顯示名稱',
    displayNamePlaceholder: '聊天中顯示的名稱…',
    contentTree: '內容',
    addPanel: '新增面板',
    panelName: '面板名稱',
    panelCaps: '允許：',
    capWrite: '改變數',
    capInsert: '填輸入框',
    capSend: '傳送',
    panelMinMessages: '滿 N 條消息後顯示面板',
  },
  definition: {
    heading: '定義',
    namePlaceholder: '定義名稱…',
    avatar: '頭像',
    searchPlaceholder: '搜尋定義，或輸入新名稱…',
    newDefinition: '+ 新增定義',
    typeLabel: '類型',
    deleteTypeTitle: '刪除類型',
    addType: '+ 類型',
    newTypePlaceholder: '新類型…',
    scanDepth: '掃描深度',
    recursive: '遞迴',
    contentPlaceholder: '定義內容…',
    htmlPreview: '預覽',
    variablesDecls: '變數',
    wrapInTag: '標籤包裹',
    wrapInTagHint: '用以此定義命名的標籤包裹內容',
    messageTypeHint: '對話開始時的開場白；其他備選作為滑動卡片。',
  },
  variables: {
    heading: '變數',
    system: '系統',
    custom: '自訂',
    namePlaceholder: '名稱',
    initialPlaceholder: '初始值',
    add: '新增變數',
  },
  settings: {
    mcp: 'MCP 工具',
    title: '設定',
    language: '語言',
    identity: '身份',
    yourName: '你的名字',
    yourNamePlaceholder: '我',
    yourAvatar: '你的頭像',
    identityHint: '當對話未附加使用者定義時，你的訊息會顯示這些資訊。',
    assistantIdentity: '助手身分',
    assistantName: '助手名稱',
    assistantNamePlaceholder: '助手',
    assistantAvatar: '助手頭像',
    assistantIdentityHint: '當對話未附加角色定義時，助手訊息會顯示這些資訊。',
    provider: '服務商',
    source: '來源',
    baseUrl: 'Base URL',
    apiKey: 'API 金鑰',
    apiKeyOptional: '本地服務商可留空',
    model: '模型',
    selectModel: '— 選擇模型 —',
    modelsHint: '填寫 Base URL 和 API 金鑰以載入模型。',
    fetching: '取得中…',
    modelsLive: '{count} 個模型',
    modelsLiveTitle: '從你的服務商即時取得',
    modelsCommon: '{count} 個常用',
    modelsCommonTitle: '內建建議 —— 填寫 API 金鑰可取得你服務商的模型',
    streamResponses: '串流回應',
    nativeTools: 'Provider 支援原生 Tool calling',
    nativeToolsSupported: '支援',
    nativeToolsUnsupported: '不支援',
    agent: 'RP Agent 框架',
    agentEnabled: '啟用 Agent 循環',
    agentTransport: 'Tool 傳輸方式',
    agentTransportAuto: '自動',
    agentTransportNative: '原生',
    agentTransportXml: 'XML 備援',
    agentRounds: '最大輪數',
    agentToolCalls: '最大 Tool 呼叫數',
    agentTimeout: 'Tool 逾時（毫秒）',
    agentRepetition: '重複呼叫輪數',
    agentActivity: '顯示執行活動',
    agentUserStatus: '顯示使用者可見狀態',
    agentTools: '能力 Tools',
    agentSystemPrompt: '框架系統提示詞',
    agentUnfinishedPrompt: '未完成時繼續提示詞',
    agentReset: '恢復預設',
    agentSessionOverride: '為此對話單獨設定',
    agentInherited: '正在使用全域 Agent 設定。',
    testing: '測試中…',
    testConnection: '測試連線',
    generation: '生成',
    temperature: '溫度',
    topP: 'Top P',
    frequencyPenalty: '頻率懲罰',
    presencePenalty: '存在懲罰',
    maxResponseTokens: '最大回應 token 數',
    maxTokensUnlimited: '不限制輸出',
    context: '上下文',
    autoSummarize: '自動總結長對話',
    contextWindow: '上下文視窗（token）',
    contextThreshold: '觸發總結（視窗佔比 %）',
    keepRecent: '保留最近訊息數',
    summarizeInstruction: '總結指令',
    notifications: '通知',
    notifyReplies: '背景回覆時通知',
    notifyUnsupported: '此應用程式/瀏覽器不支援桌面通知。',
    notifyDenied: '通知權限被拒絕。請在瀏覽器設定中允許此網站的通知，然後重試。',
    contentWidth: '內容寬度 (px)',
    appearance: '外觀',
    messageStyle: '訊息樣式',
    styleBubble: '氣泡',
    styleFlat: '平鋪',
    theme: '主題',
    themeLight: '淺色',
    themeDark: '深色',
    themeSystem: '跟隨系統',
    background: '背景',
    customCss: '自訂 CSS',
    fullscreen: '全螢幕',
    regex: '正規表示式',
    addRule: '+ 新增規則',
    about: '關於',
    aboutText: 'Shirita —— 一個由使用者掌控、可組合的 AI 角色扮演平台。',
    regexUnnamed: '未命名規則',
    regexRenameHint: '雙擊或按 Enter 重新命名',
    regexFind: '尋找',
    regexPatternPlaceholder: '正規表示式',
    regexReplace: '取代',
    regexReplacementPlaceholder: '取代文字',
    regexApplyTo: '套用至',
    regexAiOutput: 'AI 輸出',
    regexUserInput: '使用者輸入',
    regexPhaseDisplay: '僅顯示',
    regexPhaseBoth: '顯示 + prompt',
    regexPhasePrompt: '僅 prompt',
    regexUserShort: '用戶',
    regexPhase_display: '顯示',
    regexPhase_both: '兩者',
    regexPhase_prompt: 'prompt',
    regexInvalid: '失效',
    regexSearch: '搜尋規則',
    regexHideDisabled: '隱藏已停用',
    regexDelete: '刪除規則',
    regexDeleteConfirm: '刪除這條正則規則？此操作無法復原。',
  },
  mcp: {
    test: '測試',
    testOk: '已連線',
    testFailed: '失敗',
    refreshTools: '重新整理工具',
    discoveredTools: '已發現的工具',
    disabled: '停用',
    allow: '允許',
    ask: '詢問',
    none: '尚無 MCP 伺服器。',
    addServer: '新增 MCP 伺服器',
    serverId: '伺服器 ID',
    serverName: '顯示名稱',
    enabled: '啟用',
    transport: '傳輸方式',
    url: '端點 URL',
    headers: '靜態請求標頭',
    addHeader: '新增標頭',
    command: '命令',
    args: '參數（以空格分隔）',
    timeoutMs: '請求逾時（毫秒）',
  },
}

export default zhHant
