...
- **Cross-node span → folder.** Scanning left to right, if one prompt has an opening `<T>` with no matching `</T>` in its own content, and a *later* enabled in-order prompt has the matching unmatched `</T>`, the contiguous run `[opener … closer]` becomes a **`Folder` tagged `sanitize_tag(T)`** at the opener's position. Those prompts become its Ref children (in order); the literal `<T>` is stripped from the first child and `</T>` from the last. Children in a span folder are **not** individually wrapped (the folder emits `<T>…</T>`).
  - Tag token: the tag **name** is the first token after `<` (or `</`); attributes are ignored. Match `<` `/`? name `(whitespace attrs)?` `>`, where name is a run without whitespace/`<`/`>`/`/`. So `<Rule depth="0">` registers as opening `Rule` and pairs with `</Rule>`; `<最新互动>` and `<thinking>` match too. (Self-closing `<x/>` is not matched — harmless.)
...
- **`sanitize_tag` must yield a valid XML name.** Extend the shared `sanitize_tag` (`assembly.rs`) so a non-empty result whose first char is not a valid XML name-start (pragmatically: `c.is_alphabetic() || c == '_'`) gets a `tag_` prefix — e.g. `123 核心` → `tag_123_核心`; empty result stays empty (callers fall back to `def_type`/`"prompt"`). This guards both `wrap_in_tag` tags and the span/`inactive` **folder tags**, which would otherwise emit invalid markup like `<123…>`. This touches the shared helper (also used by charcard/pack wrapping), but only changes output for names that were already producing invalid XML; existing `sanitize_tag` tests (`Alice_Smith`, `主角·凛`, all-stripped→empty) are unaffected since their first chars are letters or the result is empty.
...
- `{{setvar::name::value}}` → register a `VarDecl { name, var_type, initial }` into `template.meta.variables`; **strip** the macro from the content. Infer `var_type` from the raw `value` string with the standard parses (no hand-rolled regex): `value.parse::<f64>()` ok → `Number` (`initial = json!(parsed)`); else `value.parse::<bool>()` ok → `Bool` (`initial = json!(parsed)`); else `String` (`initial = json!(value)`). Duplicate `name` across prompts: first occurrence's value is the declared `initial` (deterministic); later ones are still stripped.
...
- span with attributes: `A="<Rule depth=\"0\">foo"`, `B="bar</Rule>"` → folder tagged `Rule` (attributes ignored, opener/closer paired).
- wrap: a balanced/no-tag prompt → `wrap_in_tag: true`; a prompt with a stray `<x>` that never closes → `wrap_in_tag` absent/false (raw).
- variables: `{{setvar::hp::100}}` → `VarDecl{hp, Number, 100}`; `{{setvar::ok::true}}` → `Bool`; `{{setvar::note:: }}` → `String` (whitespace value); all stripped from content; `{{getvar::hp}}` → `{{hp}}`; an all-`setvar` prompt → no Ref but variables registered.
- `sanitize_tag`: `"123 核心"` → `"tag_123_核心"`; `"Alice Smith"` → `"Alice_Smith"` (unchanged); all-stripped → `""`.
