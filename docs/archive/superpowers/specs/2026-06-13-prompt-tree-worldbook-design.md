...
**实现：Aho-Corasick 多模式匹配。** 调研发现 SillyTavern 世界书按条目逐一扫描，约 80–100 条时性能明显劣化（已知瓶颈）。我们改用 `aho-corasick` crate（与已用的 `regex` 同作者）：
- 每次生成从**全部启用 keyword 条目**的 `keys` 构建一个自动机（`pattern → entry_id`），对扫描缓冲**一次扫描**得命中 entry 集合，复杂度 `O(缓冲长度 + 命中数)`，**与条目数解耦** —— 正面解决 ST 的 80–100 条退化。
- 递归各轮**复用同一自动机**扫描"扩充后的缓冲"。
- 扫描前统一小写缓冲与 pattern（CJK 无大小写，ASCII 由此折叠）。
- 匹配器返回**每条目命中的 key 集合**而非布尔，便于后续接 secondary keys 的逻辑运算（AND_ANY / NOT_ALL 等，对齐 ST）。
- 可选优化：按 keyword-set 哈希缓存自动机，关键词未变则免重建。
...
**结构化返回，而非拼好的字符串。** 调研显示 ST 内部用 `PromptCollection`（`Prompt{role,content,identifier}` 的有序数组）而非单一字符串，组装后再由 `prompt-converters` 做"合并相邻同角色 / 强制交替 / 折叠"等 **provider 特化序列化**。我们照此分层：

```rust
// 1) 组装 = 纯逻辑，产出有序「段」，不预拼字符串
...
enum Placement { BeforeHistory, AfterHistory }   // 后续可加 AtDepth(i32)
struct PromptSegment {
    placement: Placement,
    role: Role,        // 默认 System，后续可 per-node 配置
    content: String,   // 已渲染：容器封包 or 根 prompt 裸文本
    source: String,    // 溯源：folder tag / def id（便于测试与调试）
}
struct AssembledPlan { segments: Vec<PromptSegment> }  // 有序

// 2) 序列化 = provider 特化，拼真实消息数组
fn build_chat_messages(
    plan: &AssembledPlan,
    history: &[ChatMessage],   // 真实历史（已过滤隐藏）
    history_enabled: bool,
) -> Vec<ChatMessage>;
```
`build_chat_messages`：before 段 → 各自消息；history（启用）→ 原样插入；after 段 → 消息；末了**合并相邻同角色**（OpenAI 多 system 合一；Claude 系列必须合并/分离 system）—— 该后处理独立于组装逻辑，便于按 provider 替换。history 停用 → 跳过历史，before+after 段顺次输出后合并。
...
- 取会话有效树（自有 or 模板）+ 定义 + `override_config` + 最近消息 + 扫描配置。
- `let plan = assemble_from_nodes(...);`
- `let msgs = build_chat_messages(&plan, &history, history_enabled);`（history 含过滤隐藏后的真实消息 + 本次 user）
- 把 `msgs` 交给 provider；token 计数、SSE 流式不变。
...
```
注：此例的首条 system 实由 **3 个 before 段**（char 容器、world 容器、Main Instructions）经序列化器**合并相邻同角色**而来；`assemble_from_nodes` 内部仍是结构化的段列表。
...
**组装：** `assemble_from_nodes` + `build_chat_messages`（§8）替代 `assemble_system_prompt`；保留 `render_vars` / `apply_regex_rules` / `effective_content`。

**新依赖：** `aho-corasick`（shirita-core）用于关键词多模式匹配（§6）。
...
- **Core 单测**：`def_types` 分类与封包；Aho-Corasick 关键词匹配（多 key 命中、大小写、CJK、命中 key 集合）；触发求值（constant/keyword/random、递归开/关、scan depth）；`assemble_from_nodes` 产出**结构化段**（容器封包、根裸 prompt、placement 切分、空容器省略、局部覆盖优先、`{{var}}`）；`build_chat_messages`（段→消息、history 插入/停用、**合并相邻同角色**）；会话引用 vs fork。
...
1. **后端核心**：`def_types`（迁移+存储+API）+ history 迁移 + Aho-Corasick 关键词匹配器 + `assemble_from_nodes`（结构化段：触发/递归扫描/历史切分）+ `build_chat_messages`（段→消息 + 同角色合并）+ `conversation.rs` 改引用会话树 + 会话节点端点 + 模板/会话自动 history 节点 + 改 create_session 为引用。
...
- SillyTavern Prompt 构造：`PromptManager` / `PromptCollection` / `Prompt{role,content}` 结构化数组 + `prompt-converters` 同角色合并 —— https://deepwiki.com/SillyTavern/SillyTavern/3.3-prompt-management-and-construction ；https://docs.sillytavern.app/usage/prompts/prompt-manager/
- 注入位置（RELATIVE/ABSOLUTE depth）—— SillyTavern Prompt Manager 文档。
- 世界书扫描 `WorldInfoBuffer`（depth/recurse/inject 缓冲）、递归扫描、约 80–100 条退化 —— https://deepwiki.com/SillyTavern/SillyTavern/6.1-world-info-system
- `aho-corasick` crate（多模式匹配，`regex` 同作者）。
